...
const VocabList = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await importList(file, file.name.split('.')[0] || 'Imported List');
  ...
  const getDueWordsCount = (): number => {
    ...
    return currentList.words.filter(word => {
      const hebrewToEnglishDue = !getNextReviewDate(word, 'hebrewToEnglish') || 
                                  getNextReviewDate(word, 'hebrewToEnglish')! <= now;
      const englishToHebrewDue = !getNextReviewDate(word, 'englishToHebrew') || 
                                  getNextReviewDate(word, 'englishToHebrew')! <= now;
      return hebrewToEnglishDue || englishToHebrewDue;
    }).length;
  };
  ...
  const getFormattedReviewTimes = (word: VocabWord): React.ReactNode => {
    const hToEFormatted = formatNextReview(getNextReviewDate(word, 'hebrewToEnglish'));
    const eToHFormatted = formatNextReview(getNextReviewDate(word, 'englishToHebrew'));
    
    return (
      <div className="flex flex-col gap-1 items-end">
        <div className="flex items-center gap-1">
          <DirectionFlag direction="hebrewToEnglish" size={14} />
          <span>{hToEFormatted}</span>
        </div>
        <div className="flex items-center gap-1">
          <DirectionFlag direction="englishToHebrew" size={14} />
          <span>{eToHFormatted}</span>
        </div>
      </div>
    );
  };
